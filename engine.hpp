...
struct param_t
{
  ...
  ehfloat2 minbound = { 0,0 };
  ehfloat2 maxbound = { 1,1 };
  ...
  ehfloat2 gravity = {0,0};
};

struct particle_info_t
{
  ehfloat2 position = {0,0};
  ehfloat2 velocity = {0,0};
  ehfloat2 svelocity = {0,0};
  cl_int flag = 0;
};
...
struct engine_t
{
  ...
  // grid-base
  cl::Buffer grid_particlecount;

  // retain values on grid_sort
  cl::Buffer position, position_pong;
  cl::Buffer velocity, velocity_pong;
  cl::Buffer svelocity, svelocity_pong;
  cl::Buffer flags, flags_pong;
  cl::Buffer pressure;
  cl::Buffer V;
  cl::Buffer rho;
  cl::Buffer color, color_pong;

  cl::Buffer nonpressure_force;
  cl::Buffer pressure_force;
  cl::Buffer gradvx, gradvy;
  cl::Buffer gradvx_pong, gradvy_pong;
  cl::Buffer omega, omega_pong;
  ...
  struct
  {
    cl::KernelFunctor<
      cl::Buffer&,
      cl::Buffer&
    > gridcount_accumulate{ cl::Kernel() };
    cl::KernelFunctor<
      cl::Buffer&,
      cl::Buffer&,
      cl::Buffer&,
  ...
  struct {
    std::vector<ehfloat2> position;
    std::vector<ehfloat2> velocity;
    std::vector<ehfloat2> svelocity;
  ...
  void add_waitlist()
  {
    ...
    queue.enqueueWriteBuffer( position, CL_TRUE, sizeof(ehfloat2)*N, sizeof(ehfloat2)*n, addparticle_waitlist.position.data() );
    queue.enqueueWriteBuffer( velocity, CL_TRUE, sizeof(ehfloat2)*N, sizeof(ehfloat2)*n, addparticle_waitlist.velocity.data() );
    queue.enqueueWriteBuffer( svelocity, CL_TRUE, sizeof(ehfloat2)*N, sizeof(ehfloat2)*n, addparticle_waitlist.svelocity.data() );
  ...
  void add_particle( particle_info_t const& info )
  {
    ...
    if( info.position.s[1] > 4.5 && info.position.s[1] < 5.5 )
    {
      addparticle_waitlist.color.push_back( 1 );
    }else
    {
      addparticle_waitlist.color.push_back( 0 );
    }
